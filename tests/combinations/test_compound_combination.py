"""Unit tests for CompoundCombination class."""
...
WINNING_RANKS_SINGLE: CombinationWinningRanks = {
    (5,): 1,
    (4,): 2,
    (3,): 3,
    (2,): 4,
...
WINNING_RANKS_DUAL: CombinationWinningRanks = {
    (5, 3): 1,
    (5, 2): 2,
    (5, 1): 3,
    (5, 0): 4,
...
class TestCompoundCombination:
    """Test suite for CompoundCombination class."""

    def test_combination_empty(self):
        """Test empty CompoundCombination."""

        combination = CompoundCombination()
        assert not combination.components
        assert not combination.winning_ranks
        ...
        assert not combination.values
        assert combination.length == 0

        combination = CompoundCombination(winning_ranks=WINNING_RANKS_SINGLE)
        assert not combination.components
        assert combination.winning_ranks == WINNING_RANKS_SINGLE
        assert combination.winning_ranks is not WINNING_RANKS_SINGLE
    ...
    def test_combination_from_components(self):
        """Test CompoundCombination construction with components input."""

        numbers = Combination([5, 4, 3, 2, 1])
        extra = Combination([10, 9, 8])

        combination = CompoundCombination(numbers=numbers, winning_ranks=WINNING_RANKS_SINGLE)
        ...
        combination = CompoundCombination(extra=extra, numbers=numbers)
        ...
        combination = CompoundCombination(numbers=[5, 4, 3, 2, 1], extra=[10, 9, 8])
        assert combination.values == [1, 2, 3, 4, 5, 8, 9, 10]
        assert combination.length == 8
        assert isinstance(combination.numbers, Combination)
        assert isinstance(combination.extra, Combination)

    def test_combination_from_combination(self):
        """Test CompoundCombination construction with CompoundCombination input."""

        numbers = Combination([5, 4, 3, 2, 1])
        extra = Combination([10, 9, 8])

        original = CompoundCombination(numbers=numbers, winning_ranks=WINNING_RANKS_SINGLE)
        assert original.winning_ranks == WINNING_RANKS_SINGLE

        combination = CompoundCombination(original)
        assert combination.components == {"numbers": numbers}
        assert combination.numbers == numbers
        assert combination.winning_ranks == WINNING_RANKS_SINGLE
        assert combination.winning_ranks is not WINNING_RANKS_SINGLE
        ...
        combination = CompoundCombination(original, extra=extra)
        assert combination.components == {"numbers": numbers, "extra": extra}
        assert combination.numbers == numbers
        assert combination.extra == extra
        assert combination.winning_ranks == WINNING_RANKS_SINGLE
        ...
        original = CompoundCombination(numbers=numbers, extra=extra)
        assert not original.winning_ranks

        combination = CompoundCombination(original, winning_ranks=WINNING_RANKS_SINGLE)
        assert combination.components == {"numbers": numbers, "extra": extra}
        assert combination.numbers == numbers
        assert combination.extra == extra
        assert combination.winning_ranks == WINNING_RANKS_SINGLE
    ...
    def test_combination_raise_on_invalid_component(self):
        """Test CompoundCombination raises ValueError on invalid component."""

        with pytest.raises(ValueError):
            _ = CompoundCombination(numbers="invalid_component")

        with pytest.raises(ValueError):
            _ = CompoundCombination(numbers=42)

        with pytest.raises(AttributeError):
            _ = CompoundCombination().numbers
    ...
    def test_get_combination_factory(self):
        ...
        def custom_factory(
            ...
            **components,
            ...
            return CompoundCombination().get_combination(combination, **components)

        factory = CompoundCombination.get_combination_factory(custom_factory)
    ...
    def test_combination_copy(self):
        """Test CompoundCombination copy method."""

        numbers = Combination([3, 5, 18, 29, 42])
        numbers2 = Combination([6, 9, 12])
        extra = Combination([10, 9, 8])

        combination = CompoundCombination(
            numbers=numbers, extra=extra, winning_ranks=WINNING_RANKS_SINGLE
        )
        ...
        combination2 = combination.copy(winning_ranks=WINNING_RANKS_DUAL)
        assert isinstance(combination2, CompoundCombination)
        assert combination2 is not combination
        assert combination2.components == combination.components
        assert combination2.numbers == combination.numbers
        assert combination2.extra == combination.extra
        assert combination2.winning_ranks == WINNING_RANKS_DUAL
        assert combination2.values == combination.values
        assert combination2.length == combination.length
        ...
        assert combination2.extra == combination.extra
        assert combination2.winning_ranks == combination.winning_ranks
        assert combination2.values == numbers2.values + extra.values
        assert combination2.length == extra.length + numbers2.length

    def test_combination_get_combination(self):
        """Test CompoundCombination get_combination method."""
        ...
        new_combination = combination.get_combination(
            combination2, winning_ranks=WINNING_RANKS_DUAL
        )
        assert isinstance(new_combination, CompoundCombination)
        assert new_combination.values == [2, 3, 4, 5, 6, 7, 8]
        assert new_combination.winning_ranks == WINNING_RANKS_DUAL
        ...
        new_combination = combination.get_combination(
            numbers=[6, 7, 8, 9, 10], extra=[15, 16, 17], winning_ranks=WINNING_RANKS_DUAL
        )
        assert isinstance(new_combination, CompoundCombination)
    ...
    def test_combination_getting_components(self):
        """Test CompoundCombination getting components."""
        ...
        assert components == {"numbers": numbers.copy(values=[4, 5, 6, 7, 8])}

        components = combination.get_components(extra=[18, 19, 20])
        assert components == {"extra": extra.copy(values=[18, 19, 20])}
    ...
    def test_combination_getting_values(self):
        """Test CompoundCombination getting values."""
    ...
    def test_combination_get_winning_rank(self):
        ...
        combination = CompoundCombination(
            numbers=numbers, extra=extra, winning_ranks=WINNING_RANKS_DUAL
        )
    ...
    def test_combination_equality(self):
        """Test CompoundCombination equality comparisons."""

        numbers = Combination([1, 2, 3, 4, 5])
        extra = Combination([6, 7, 8])

        numbers_rank = numbers.rank
        ...
        assert CompoundCombination() == CompoundCombination()
        assert CompoundCombination().equals()
    ...
    def test_combination_includes(self):
        """Test CompoundCombination includes method."""

        numbers = Combination([1, 2, 3, 4, 5])
        extra = Combination([6, 7, 8])

        combination1 = CompoundCombination(numbers=numbers, extra=extra)
        combination2 = CompoundCombination(numbers=Combination([1, 2]))
        combination3 = CompoundCombination(numbers=Combination([2, 6]))
    ...
    def test_combination_intersects(self):
        """Test CompoundCombination intersects method."""

        numbers = Combination([1, 2, 3, 4, 5])
        extra = Combination([6, 7, 8])

        combination1 = CompoundCombination(numbers=numbers, extra=extra)
        combination2 = CompoundCombination(numbers=Combination([1, 3, 5, 6, 7]))
        combination3 = CompoundCombination(numbers=Combination([6, 7, 8, 9, 10]))
    ...
    def test_combination_intersection(self):
        """Test CompoundCombination intersection method."""
        ...
        combination1 = CompoundCombination(numbers=numbers, extra=extra)
        combination2 = CompoundCombination(numbers=Combination([1, 3, 5, 6, 7]))
        combination3 = CompoundCombination(numbers=Combination([1, 3, 5, 6, 7]), extra=extra)
        combination4 = CompoundCombination(numbers=Combination([6, 7, 8, 9, 10]))

        intersection1 = combination1.intersection([1, 3, 5, 6, 7])
        intersection2 = combination1.intersection([6, 7, 8, 9, 10])

        assert isinstance(intersection1, CompoundCombination)
        assert isinstance(intersection2, CompoundCombination)
    ...
    def test_combination_compares(self):
        """Test CompoundCombination compares method."""
        ...
        combination1 = CompoundCombination(numbers=numbers, extra=extra)
        combination2 = CompoundCombination(numbers=Combination([1, 3, 5, 6, 7]))
        combination3 = CompoundCombination(combination1)

        assert combination1.compares() == 1
        assert CompoundCombination().compares(combination1) == -1
        assert CompoundCombination().compares() == 0
    ...
    def test_combination_similarity(self):
        """Test CompoundCombination similarity method."""
        ...
        combination1 = CompoundCombination(numbers=numbers, extra=extra)
        combination2 = CompoundCombination(numbers=Combination([1, 3, 5, 6, 7]))
        combination3 = CompoundCombination(
            numbers=Combination([6, 7, 8, 9, 10]),
            extra=extra,
        )

        assert CompoundCombination().similarity() == 1
        assert CompoundCombination().similarity([1, 2, 3, 4, 5]) == 1
    ...
    def test_combination_iteration(self):
        """Test CompoundCombination iteration."""
    ...
    def test_combination_string(self):
        """Test CompoundCombination string representation."""

        numbers = Combination([3, 6, 12, 33, 42])
        extra = Combination([6, 7, 12])
    ...
    def test_combination_repr(self):
        """Test CompoundCombination repr representation."""
        ...
        numbers_repr = "Combination(values=[1, 2, 3, 4, 5], rank=None, start=1)"
        extra_repr = "Combination(values=[6, 7, 8], rank=None, start=1)"
        winning_ranks_repr = "{}"

        assert repr(combination) == (
            f"CompoundCombination(numbers={numbers_repr}, "
            f"extra={extra_repr}, "
            f"winning_ranks={winning_ranks_repr})"
        )

        assert repr(CompoundCombination()) == (
            f"CompoundCombination(winning_ranks={winning_ranks_repr})"
        )

    def test_combination_is_unhashable(self):
        """Test CompoundCombination is intentionally unhashable."""

        combination = CompoundCombination(numbers=[1, 2, 3, 4, 5])
    ...
    def test_combination_to_json_equals_to_dict(self):
        ...
        extra = Combination([6, 7, 8])
        combination = CompoundCombination(
            numbers=main, extra=extra, winning_ranks=WINNING_RANKS_DUAL
        )
    ...
    def test_combination_to_dict_includes_components_and_winning_ranks(self):
        ...
        assert result["winning_ranks"] == WINNING_RANKS_DUAL
    ...
    def test_combination_from_string(self):
        ...
        assert isinstance(combination, CompoundCombination)
        assert list(combination.components.keys()) == ["numbers", "extra"]
    ...
    def test_combination_from_string_without_brackets(self):
        """Test from_string creates a CompoundCombination from a string without brackets."""
    ...
    def test_combination_from_csv(self):
        """Test from_csv creates a CompoundCombination from a CSV-compatible dictionary."""
    ...
    def test_combination_from_json_equals_from_dict(self):
        ...
        data = {
            ...
            "winning_ranks": WINNING_RANKS_SINGLE,
        }

        from_json = CompoundCombination.from_json(data)
        from_dict = CompoundCombination.from_dict(data)
    ...
    def test_combination_from_dict(self):
        """Test from_dict creates a CompoundCombination from a dictionary."""

        data = {
            "components": {
                "numbers": {"values": [1, 2, 3, 4, 5], "rank": None, "start": 1},
                "extra": {"values": [6, 7, 8], "rank": None, "start": 1},
            },
            "winning_ranks": WINNING_RANKS_DUAL,
        }

        combination = CompoundCombination.from_dict(data)

        assert list(combination.components.keys()) == ["numbers", "extra"]
        assert combination.components["numbers"].values == [1, 2, 3, 4, 5]
        assert combination.components["extra"].values == [6, 7, 8]
        assert combination.winning_ranks == WINNING_RANKS_DUAL

    def test_combination_from_dict_empty_data(self):
        """Test from_dict with empty dictionary creates an empty CompoundCombination."""

        combination = CompoundCombination.from_dict({})
    ...
    def test_combination_serialization_roundtrip(self):
        ...
        main = Combination([3, 15, 22, 28, 44])
        extra = Combination([2, 5, 8])
        original = CompoundCombination(numbers=main, extra=extra, winning_ranks=WINNING_RANKS_DUAL)

        restored = CompoundCombination.from_dict(original.to_dict())
